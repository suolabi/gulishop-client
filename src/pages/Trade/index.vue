  ...
  export default {
    ...
    async submitOrder() {
      ...
      let tradeInfo = {
        ...
        consigneeTel: this.defaultAddress.phoneNum, //创建订单所需要的用户名字
      ...
      if (result.code === 200) {
        ...
        //  请求成功，返回一个订单编号，在跳转到支付页面，接着携带着订单编号到支付页面
        alert('创建订单成功，自动跳转到支付页面')
        //接着携带着订单编号到支付页面
        this.$router.push('/pay?orderNo=' + result.data) //拼接字符串写法
        // this.$router.push(`/pay?orderNo= ${result.data}`) //模板字符串
      }
    },
    },
    computed:{
      ...
      // 在userAddressList这个数组中查找，isDefault = 1的项
      defaultAddress(){
